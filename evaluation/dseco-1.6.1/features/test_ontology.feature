...
Feature: Functional test on a test data graph structured by the DSecO ontology.

  Scenario: I have many FQDN with a ZONE which is example.org
    Given that the KG is loaded and the reasoner applied
    When I run SPARQL query located at queries/test_example.org_entries/test.sparql to gather entries with ZONE is example.org
  ...
  Scenario: Récupérer les entrées de la zone example.org avec leur résolution finale
    Given that the KG is loaded and the reasoner applied
    When I run SPARQL query located at queries/test_entries_with_resolution/test.sparql to gather entries with ZONE is example.org and their final resolution
